 ...
 */
public class MyListFragmentIT extends ListFragment implements LoaderManager.LoaderCallbacks<String[][]> {// LoaderCallbacksのジェネリクスには、Loaderの戻り値の型を指定する(今回はString)。Workerを作成、実行、結果を受け取るクラス。
...
/* ここから*/
   /*///////     List<Item> objects = new ArrayList<Item>();/////
    ...
    @Override
    public RSSLoaderIT onCreateLoader(int arg0, Bundle arg1) {//ワーカーの作成など、非同期処理の初期化
        return new RSSLoaderIT(getActivity());  //
    }
